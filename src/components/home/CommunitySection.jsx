...
export default function CommunitySection() {
  return (
    <section className="py-28 bg-[#312E81]" dir="rtl">
      <div className="max-w-7xl mx-auto px-8">
        <div className="flex flex-col lg:flex-row gap-14 items-center">
          {/* Content */}
          <div className="flex-1 text-right">
            <span className="text-primary-foreground/50 font-semibold text-sm mb-4 block uppercase tracking-wider">הקהילה שלנו</span>
            <h3 className="text-4xl font-extrabold mb-6 text-white">קהילת ה-New Gen</h3>
            ...
            <div className="flex items-center gap-4 mb-10">
              <div className="flex -space-x-3 space-x-reverse">
                {avatars.map((src, i) => (
                  <img key={i} className="w-11 h-11 rounded-full border-[3px] border-[#312E81] object-cover" alt="Profile" src={src} />
                ))}
                <div className="w-11 h-11 rounded-full border-[3px] border-[#312E81] bg-accent flex items-center justify-center text-xs font-bold text-white">
                  +2k
                </div>
              </div>
              <span className="text-white/35 text-sm">משתמשים פעילים</span>
            </div>
            <Link to={createPageUrl("Questionnaire")}>
              <button className="bg-white text-[#312E81] px-9 py-4 rounded-2xl font-bold text-sm hover:bg-white/90 transition-all duration-300 flex items-center gap-2 shadow-lg shadow-black/10">
          ...
          <div className="grid grid-cols-2 gap-4 flex-shrink-0">
            {[
              ...
              <div key={stat.label} className="bg-white/[0.06] backdrop-blur-sm rounded-2xl border border-white/[0.06] p-7 text-center w-36">
                <div className="text-3xl font-black text-white mb-1.5">{stat.value}</div>
