...
const getMasterPrompt = (language, userName, userGender, boosterScores, selectedTrack, optionalComment) => {
  ...
  if (language === 'he') {
    ...
    return `אתה יועץ אסטרטגי בכיר. צור דוח V107 מקצועי, חד וישיר עבור ${userName} (${userGender === 'female' ? 'נקבה' : 'זכר'}).

הנחיות:
1. התאם את כל הפעלים למגדר ${userGender === 'female' ? 'נקבה' : 'זכר'}
2. אסור להשתמש ב: "יזם", "יזמות", "כישלון"
3. השתמש ב: "מנוע פנימי", "מלכודת ה...", "פרדוקס", "The Wake-up Call"
4. טון: ישיר, חד, אמיץ, ממוקד

נתונים:
${scoresText}
מסלול בוסטר: ${selectedTrackName}
${optionalComment ? `הערה: ${optionalComment}` : ''}

צור 5 עמודים:

# עמוד 1: The Wake-up Call
פתיחה: ${userName}, ${userGender === 'female' ? 'את' : 'אתה'} [ניתוח המנוע הביצועי]
תובנת עומק: [חשיפת הפרדוקס המרכזי]
הבהרה משפטית מקוצרת
...
# עמוד 2: המנוע הפנימי
3-4 נקודות:
• מלכודת ה"[שם]": [איך דפוס חיובי הופך הרסני]
• הפרדוקס המקצועי: [החוזקה=החסם]

---

# עמוד 3: The Mirror
ניתוח ROI אישי
ציוני ליבה (מילולי בלבד): ביצוע, ניהול, אסטרטגיה, פיננסים, דיגיטל, יחסי אנוש

---

# עמוד 4: המלצות
${userName}, ${userGender === 'female' ? 'את' : 'אתה'} [פתרון אמיתי]
3-4 נתיבי מימוש
ארגז כלים: ספר, כלי, פרוטוקול 24 שעות
