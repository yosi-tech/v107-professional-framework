...
3. 🔴 אסור בהחלט: אל תזכיר "5,200 משתמשים" / "בסיס נתוני V107" / "X% מהמשתמשים" / "אלפי משתמשים" / "מאגר נתונים" / כל התייחסות למספר משתמשים או בסיס נתונים. אין למערכת בסיס נתוני משתמשים. כל הזכרה כזו = דוח פסול מיידי.
...
8. 🔴 תזכורת חוזרת: אין מאגר משתמשים, אין "5,200", אין "אלפי", אין "בסיס נתונים". הפרה = דוח פסול.
...
פתח בתיבה ממוסגרת בפורמט הזה:
---
### 📋 תקציר מנהלים — [name] | V107 REPORT
---
🏆 **הפרופיל שלך:** [archetype.hebrew_name]
⚡ **שני הכוחות:** [top3[0].name_he] ([top3[0].score]) + [top3[1].name_he] ([top3[1].score])
...
מיד אחריה — תיבת מקינזי בפורמט הזה:
...
גרף — העתק בדיוק מ-chart_data שב-JSON בפורמט הזה:
---
### 🗺️ מפת יכולות — [name] | V107
---
